...
*Status: D2+D5 VALIDATED AND IMPLEMENTED (2026-07-19); D3 tested and
rejected; D1/D4 deferred. Drafted 2026-07-19 from rashi-search calibration
round 1 (4/10 exact top-1; all 5 misses are short 2–4-word dibburim built
from common vocabulary). Follow the v2 discipline: corpus experiments first
(`experiments/`), competition ranks (strict rank + tie-group size), no
...
## Sweep results (2026-07-19)

Grid swept `coverageWeight` (D2, with and without a `fullFieldBonus` variant),
`refineWeightLatin` (D5), and D3 popularity-bin sort, against the 195-query
synthetic set (main) plus the partial-dibbur regression set, over
`rashi-search` harness `scripts/eval.mjs` + `scripts/sweep.mjs`.

**Winner: `coverageWeight = 20`, `refineWeightLatin = 40`.** `fullFieldBonus`
and D3 rejected.

| metric | baseline (v2) | winner (D2+D5) |
|---|---|---|
| main top-1 | 71.0% | 86.6% |
| main top-3 | — | 94.9% |
| main miss rate | — | 0.0% |
| 1–2-word-dh bucket top-1 | 28.1% | 84.2% |
| partial-dibbur set top-1 | 50% | 45% |

Curve shapes:
- `coverageWeight`: gains ramp up quickly then **plateau across 20–80**;
  picked 20 as the smallest value capturing the full gain (minimizes risk of
  a coverage bonus crossing a tier it shouldn't on some future corpus).
- `fullFieldBonus`: no gain on top of coverageWeight alone, and **hurt** the
  combined (main + partial) score when stacked with coverage — rejected
  rather than shipped as a no-op knob.
- `refineWeightLatin`: **peaked at 40**; scores **declined by 75**, so this
  is a real optimum, not a monotonic "more is better" knob.
- D3 popularity-bin sort: no effect once D2/D5 are in place (the evidence
  spread D2/D5 add already separates former near-ties enough that
  popularity-bin quantization has nothing left to do) — in a few configs it
  was mildly harmful. Rejected; the ranking is left as an ordinary sort with
  no binning.

**Caveats:**
1. Eval vowels are engine-perfect (generated by the transliteration engine
   itself, not typed by a human under uncertainty) — the refineWeightLatin
   win may be partly an artifact of clean vowel evidence. Before raising
   refineWeightLatin further, add vowel-noise query variants (dropped/wrong
   vowels) to the synthetic set and re-sweep.
2. The partial-dibbur set costs about 1 query (50% → 45% top-1) — coverage
   rewards fields fully consumed by the query, which structurally
   disadvantages a query that's a genuine prefix of a longer dibbur. Judged
   an acceptable trade for the large gain on the main/short-query sets;
   revisit if real users complain about prefix-of-long-dibbur queries.

## D3 postscript: live check (2026-07-19, later)

The harness stubs the Sefaria calendar, so the sweep never saw **weekBoost**.
A live browser check during Va'etchanan week exposed the gap: "saviv" ranked
five current-parsha docs above the two סביב dibburim. Root cause is not a
bug — יסף folds to the same coarse key `SB` as סביב (b/v/p/f merge), an
exact-tier skeleton-mate that v3's evidence correctly discounts by ~20 pts
(refinedSim 0.75 vs 1.0, coverage 0.5 vs 1.0) — but weekBoost was 40, bigger
than the whole evidence quantum.

Two fixes were tested and REJECTED before the real one:
- **Bin sort (D3 option 1), any bin size 6–15:** order-based top-1 collapsed
  to 46% — bins put dozens of skeleton-mates in one bin and hand the decision
  to popularity. The earlier "bin=15 is neutral" sweep row was a
  rank-metric artifact (strict rank is computed from scores, not sort order).
- **Shrinking/zeroing popBoost:** popBoost at its current scale (log2·3.5,
  real-world max ~25) is *earning* its keep — zeroing it costs 26 pts of
  top-1 on short queries. Among docs with identical dibburim it is the right
  tie-breaker. Left unchanged.

The actual fix (rashi-search side): **weekBoost rescaled 40/20 → 8/4** —
below the ~10-pt vowel-evidence quantum, so it breaks true ties but can no
longer let a current-parsha skeleton-mate leapfrog a vowel-perfect match.
Verified live: "saviv" → both סביב docs first, ולא יסף third. General
lesson for chipus apps: additive app-side boosts must stay below the
smallest evidence gap the engine produces (~REFINE_WEIGHT × 0.25), or they
stop being tie-breakers and start being votes.
